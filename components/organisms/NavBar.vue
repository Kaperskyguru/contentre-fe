<template>
  <div class="lg:flex lg:justify-center relative lg:absolute w-full lg:top-8">
    <div id="auth-navbar" class="flex items-center justify-between w-full py-4 pr-4 pl-2 lg:w-80vw">
      ...
      <div id="navDropdown" class="flex hidden lg:flex lg:flex-row flex-col w-full lg:w-max absolute lg:static top-full bg-white lg:bg-transparent px-3 pt-3 left-0 lg:pt-0 lg:pr-4">
        <NuxtLink v-for="link in links" :key="link.text" :to="link.to" class="pb-3 first:mt-5 lg:pb-0 lg:pl-3 lg:first:pl-0 lg:mt-0">
          <div class="flex">
            <img class="pr-1" :src="link.icon" :alt="link.text"/>
            <span class="text-helvetica text-bold text-xs">{{ link.text.toUpperCase() }}</span>
          </div>
        </NuxtLink>
      </div>
    </div>
